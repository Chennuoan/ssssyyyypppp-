...
# 数据绘图
def plot_data(data, output_dir="plots2"):
    try:
...
def run_model(data):
    try:
        with pm.Model() as model:
            ...
            # 线性模型
            mu = intercept + beta_ozone * data['ozone'] + beta_temp * data['temperature'] + beta_humidity * data[
                'relative_humidity']
        ...
        plt.savefig("plots2/posterior_distribution.png", dpi=300, bbox_inches="tight")
        plt.close()
        # 设置图形的大小
        ...
        plt.savefig("plots2/trace_plots.png", dpi=300, bbox_inches="tight")
        plt.close()
...
def posterior_predictive_plot_with_CI(data, model, trace, output_dir="plots2"):
    try:
        # 获取后验预测
        with model:
            # 采样所有变量
...
def posterior_predictive_plot(data, model, trace, output_dir="plots2"):
    try:
        # 获取后验预测
        with model:
            # 采样所有变量
...
def main(file_path='londondataset2002_2006.csv', output_dir="plots2"):
    data = load_data(file_path)
    plot_data(data, output_dir)

    try:
        trace, model, mu = run_model(data)
        inference_data = pm.to_inference_data(trace, model=model)
