...
# 假设 co2_by_month 和 num_forecast_steps 已经正确定义
# co2_by_month = ...  # 你的数据
# num_forecast_steps = ...  # 你定义的预测步数

# Ensure the data is in TensorFlow format
co2_by_month_training_data = tf.convert_to_tensor(co2_by_month[:-num_forecast_steps], dtype=tf.float32)
...
num_variational_steps = 200  # 设置优化步数
...
print("Inferred parameters:")
for param_name in variational_posteriors:
    # Assuming the variational posterior is a dictionary-like object
    param_mean = np.mean(q_samples_co2_[param_name], axis=0)
    param_std = np.std(q_samples_co2_[param_name], axis=0)
    print(f"{param_name}: {param_mean} ± {param_std}")

# Optionally: You can plot the parameters if needed
# plt.plot(param_mean, label="Mean")
# plt.plot(param_std, label="Standard Deviation")
# plt.legend()
# plt.show()
